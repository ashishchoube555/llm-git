...
tools=[get_word_length,get_upper_word,tavily_tool]
...
agent_executor = AgentExecutor(agent=agent, tools=tools, verbose=True, handle_parsing_errors=True,Callbacks=CallbackManager,early_stopping_method="force")
response = agent_executor.invoke({"input": [HumanMessage(content=query5)]})
print(response)
